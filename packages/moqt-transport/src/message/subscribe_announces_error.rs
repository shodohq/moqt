...
/// Representation of a SUBSCRIBE_ANNOUNCES_ERROR message body.
#[derive(Debug, PartialEq, Eq, Clone)]
pub struct SubscribeAnnouncesError {
    /// The request ID of the SUBSCRIBE_ANNOUNCES message this is replying to.
    pub request_id: u64,
...
mod tests {
    ...
    fn encode_decode_roundtrip_with_reason() {
        let msg = SubscribeAnnouncesError {
            request_id: 42,
            error_code: 0x4,
            error_reason: "namespace prefix unknown".into(),
        };
        ...
        let decoded = SubscribeAnnouncesError::decode(&mut decode_buf).unwrap();
